    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result?.error) {
                ...
                // After successful login, fetch fresh session to get user role
                ...
                if (session?.user?.role && ADMIN_ROLES.includes(session.user.role)) {
                    ...
                    console.log("Redirecting to /profile", session?.user);
                    // router.push("/profile");
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="text-center mb-8">
                <h2 className="text-2xl font-display text-white mb-2">স্বাগতম!</h2>
                <p className="text-white/60 text-sm">আপনার একাউন্টে প্রবেশ করুন</p>
            </div>
                    ...
                    <label className="flex items-center gap-2 cursor-pointer group">
                        <div className="relative">
                            ...
                            <div className="w-5 h-5 border border-white/20 rounded bg-white/5 peer-checked:bg-gold-500 peer-checked:border-gold-500 transition-all duration-300 flex items-center justify-center">
                                <svg
                                    className="w-3 h-3 text-primary-950 opacity-0 peer-checked:opacity-100 transition-opacity"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    stroke="currentColor"
                                    strokeWidth={3}
            ...
            <div className="relative my-8">
                ...
                <div className="relative flex justify-center text-sm">
                    <span className="px-4 bg-transparent text-white/40">অথবা</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                <button
                    type="button"
                    className="flex items-center justify-center gap-2 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-white/80 hover:text-white transition-all duration-300"
                >
