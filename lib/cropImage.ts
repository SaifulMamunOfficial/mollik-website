...
export default async function getCroppedImg(
    ...
    // Resize if too large (max 700x700)
    const MAX_SIZE = 700;
    ...
    // Iterative compression to get under 200KB
        ...
        const compress = () => {
            finalCanvas.toBlob(
                (blob) => {
                    ...
                    // If file is small enough or quality is already low, return it
                    if (blob.size <= 200 * 1024 || quality <= 0.5) {
                        // console.log(`Final size: ${(blob.size / 1024).toFixed(2)} KB, Quality: ${quality}`);
                        resolve(blob);
                    } else {
